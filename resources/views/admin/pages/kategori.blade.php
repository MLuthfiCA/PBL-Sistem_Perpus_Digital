...
<div class="py-6 sm:py-10 space-y-6 sm:space-y-8">
    ...
    <form action="{{ route('admin.kategori.index') }}" method="GET" class="p-4 sm:p-6 bg-white/80 rounded-2xl shadow-xl">
        <div class="flex flex-col md:flex-row gap-3 sm:gap-4 items-center">
            ...
            <div class="flex items-center gap-2 sm:gap-3 w-full md:w-auto">
                ...
                <a href="{{ route('admin.kategori.index') }}" class="flex-1 md:flex-initial px-6 py-2.5 bg-gray-200 text-gray-700 rounded-xl hover:bg-gray-300 font-bold text-sm transition-all text-center">Reset</a>
            </div>
        </div>
    </form>
            ...
            <table class="w-full text-left border-collapse min-w-[500px]">
                <thead class="bg-red-50/50 text-gray-400 text-[10px] font-bold uppercase tracking-widest">
                    <tr>
                        ...
                        <th class="px-6 sm:px-8 py-4 sm:py-5">Category Name</th>
                ...
                <tbody class="divide-y divide-red-50">
                    ...
                    <tr class="group hover:bg-red-50/30 transition-colors duration-300">
                        <td class="px-6 sm:px-8 py-4 sm:py-6 text-sm">{{ $kategori->id_kategori }}</td>
                        <td class="px-6 sm:px-8 py-4 sm:py-6 font-bold text-gray-700 text-sm">{{ $kategori->nama_kategori }}</td>
                        <td class="px-6 sm:px-8 py-4 sm:py-6 text-right opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                            <button onclick="editCategory({{ $kategori->id_kategori }}, '{{ $kategori->nama_kategori }}')" class="text-blue-500 hover:text-blue-700 font-bold text-[10px] sm:text-xs px-2 sm:px-3 bg-blue-50 sm:bg-transparent rounded py-1 sm:py-0 mr-1 sm:mr-2">Edit</button>
                    ...
                    @empty
                    <tr>
                        <td colspan="3" class="px-8 py-12 text-center text-gray-400 font-medium text-sm">No categories found.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
...
<div id="addModal" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-2xl shadow-xl p-6 sm:p-8 w-full max-w-md">
        <h2 class="text-xl sm:text-2xl font-bold mb-4 sm:mb-6">Add Category</h2>
        <form action="{{ route('admin.kategori.store') }}" method="POST">
            @csrf
            <div class="mb-4">
                <label class="block text-[10px] sm:text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Category Name</label>
                <input type="text" name="nama_kategori" required class="w-full px-4 py-2 border rounded-xl focus:ring-2 focus:ring-red-200 outline-none text-sm">
            </div>
...
<div id="editModal" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-2xl shadow-xl p-6 sm:p-8 w-full max-w-md">
        <h2 class="text-xl sm:text-2xl font-bold mb-4 sm:mb-6">Edit Category</h2>
        <form id="editForm" method="POST">
            @csrf @method('PUT')
            <div class="mb-4">
                <label class="block text-[10px] sm:text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Category Name</label>
                <input type="text" name="nama_kategori" id="edit_nama_kategori" required class="w-full px-4 py-2 border rounded-xl focus:ring-2 focus:ring-red-200 outline-none text-sm">
            </div>
            <div class="flex justify-end gap-2 mt-6">
