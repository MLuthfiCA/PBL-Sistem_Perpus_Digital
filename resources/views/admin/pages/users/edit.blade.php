...
<div class="py-10 space-y-8" x-data="{}">
    <x-ui.page-header title="Edit Pengguna" subtitle="Ubah data pengguna"></x-ui.page-header>
    <div class="p-6 bg-white/80 rounded-2xl shadow-xl">
        <form action="{{ route('admin.users.update', $user->user_id) }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                <label class="block text-sm font-bold text-gray-700 mb-1">Password (kosongkan bila tidak diubah)</label>
            ...
            <div class="col-span-2 flex justify-end space-x-4">
                <a href="{{ route('admin.users.index') }}" class="px-6 py-2.5 bg-gray-300 text-gray-800 rounded-xl hover:bg-gray-400">Batal</a>
                <button type="submit" class="px-6 py-2.5 bg-burgundy-500 text-white rounded-xl hover:bg-maroon">Simpan Perubahan</button>
            </div>
        </form>
    </div>
</div>
