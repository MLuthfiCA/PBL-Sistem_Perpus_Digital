    ...
    <div class="p-6 bg-white/80 rounded-2xl shadow-xl">
        <form action="{{ route('admin.users.update', $user->user_id) }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="identity_number" inputmode="numeric" pattern="[0-9]*" oninput="this.value = this.value.replace(/[^0-9]/g, '');" class="w-full px-4 py-2.5 border rounded" value="{{ $user->identity_number }}" placeholder="Enter Student ID (NIM)" required>
            </div>
            <div>
            ...
            <div>
                ...
                <select name="role" class="w-full px-4 py-2.5 border rounded" required>
                    ...
                    <option value="student" {{ $user->role == 'mahasiswa' ? 'selected' : '' }}>Student</option>
                </select>
            </div>
            <div>
